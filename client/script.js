document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupBookingForm = () => { /* ... obsah funkce beze změny ... */ };
    ...
    const initializeApp = async () => {
        try {
            ...
            // --- UPRAVENÝ DIAGNOSTICKÝ BLOK ---
            console.log('--- DIAGNOSTIKA DAT ---');
            console.log('Načtené překlady:', translations);
            console.log('Načtená konfigurace:', publicConfig);
            console.log('---------------------');
            // ------------------------------------
...
// Zde vložte zbytek obsahu funkce setupBookingForm, který je beze změny
const _setupBookingForm = `
...
`;
// Vložení kódu do funkce, aby se předešlo duplikaci
(function() {
    const fnBody = _setupBookingForm.substring(_setupBookingForm.indexOf("{") + 1, _setupBookingForm.lastIndexOf("}"));
    const setupBookingFormFn = new Function('minOrderDate', 'unavailableDates', 'translations', 'currentLang', 'publicConfig', 'lastCalculatedPrice', 'API_BASE_URL', fnBody);

    // Přepsání původní prázdné funkce
    window.setupBookingForm = function() {
        setupBookingFormFn.call(this, ...arguments);
    };
})();